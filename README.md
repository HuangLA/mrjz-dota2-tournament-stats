# MRJZ - 节奏英雄勺大赛数据统计平台

为"节奏英雄勺大赛"提供专业的数据统计分析平台，包含比赛详情、选手数据、成就系统等功能。

## 技术栈

### 前端
- React 18
- React Router
- Ant Design
- Axios
- Vite

### 后端
- Node.js + Express.js
- Sequelize (MySQL ORM)
- node-cron (定时任务)
- OpenDota API / Steam API

### 数据库
- MySQL 8.0 / MariaDB

## 项目结构

```
MRJZ/
├── aio/                 # All-in-One Docker 构建文件
├── backend/             # Node.js 后端服务
│   ├── src/
│   │   ├── config/      # 配置文件
│   │   ├── models/      # Sequelize 模型
│   │   ├── controllers/ # 控制器
│   │   ├── routes/      # 路由
│   │   ├── services/    # 业务逻辑
│   │   └── jobs/        # 定时任务
├── frontend/            # React 前端
│   ├── src/
│   │   ├── components/  # 通用组件
│   │   ├── pages/       # 页面组件
└── docker-compose.yml   # Docker 部署配置
```

## 快速开始

### 🐳 方式 1: All-in-One Docker 部署（推荐）

本项目采用 **All-in-One (三合一)** 容器架构，将前端(Nginx)、后端(Node.js)和数据库(MariaDB)打包在一个容器中，极大简化了部署流程，特别适合 NAS 部署。

**前置要求**: 只需安装 [Docker Desktop](https://www.docker.com/products/docker-desktop)

1. **准备配置文件**
   在项目根目录创建一个 `.env` 文件（或直接修改 `docker-compose.yml` 中的环境变量）：
   ```bash
   # Steam API Key (必填，从 https://steamcommunity.com/dev/apikey 获取)
   STEAM_API_KEY=你的SteamWebAPIKey
   
   # 联赛 ID (默认 18365)
   LEAGUE_ID=18365
   ```

2. **启动服务**
   ```bash
   docker-compose up -d
   ```

3. **访问应用**
   容器启动后（首次启动需等待数据库初始化），访问：
   - **前端**: http://localhost:5173
   - **数据库数据**: 自动持久化在 Docker 卷 `mrjz-data` 中

---

### 💻 方式 2: 传统部署

#### 环境要求

- Node.js >= 16
- MySQL >= 8.0
- npm 或 yarn

#### 🗄️ 数据库准备 (重要)

在启动后端之前，必须手动设置本地 MySQL 数据库：

1. 确保 MySQL 服务已运行 (推荐版本 8.0+)
2. 连接到 MySQL 并执行以下脚本：
```bash
# 1. 创建数据库
source database/init.sql

# 2. 创建表结构
source database/tables.sql
```

#### 后端启动

```bash
cd backend
npm install

# 配置环境变量 (必须)
cp .env.example .env
# ⚠️ 编辑 .env 文件，填入你的 MySQL 密码 和 Steam API Key
# nano .env 

npm start
```

服务器将在 `http://localhost:3001` 启动

#### 前端启动

```bash
cd frontend
npm install
npm run dev
```

前端将在 `http://localhost:5173` 启动

---

## API 文档

### 基础信息

**Base URL**: `http://localhost:3001/api`

**响应格式**: JSON

### 比赛相关 API

#### 1. 获取比赛列表

```
GET /api/matches?league_id={leagueId}&page={page}&pageSize={pageSize}
```

#### 2. 获取比赛详情

```
GET /api/matches/:matchId
```

#### 3. 强制刷新比赛数据 (管理功能)

```
POST /api/matches/force-refresh?league_id={leagueId}
```

### 选手相关 API

#### 1. 获取选手列表
```
GET /api/players?page={page}&pageSize={pageSize}
```

#### 2. 获取选手详情
```
GET /api/players/:playerId
```

#### 3. 获取选手英雄统计
```
GET /api/players/:playerId/heroes
```

---

## 功能特性

- ✅ 比赛详情展示（参考 Dotabuff）
  - 完整的玩家数据（KDA、GPM、XPM、伤害等）
  - 经济数据（正补、反补、净值）
  - 装备显示（主装备、背包、中立物品）
  - 路线信息
- ✅ 全部比赛列表（支持分页和筛选）
- ✅ 选手详情页面（统计数据、英雄池）
- ✅ 成就系统（8种成就类型）
- ✅ 数据同步功能
  - 增量同步（只同步新比赛）
  - 强制刷新（删除后重新同步）
  - 定时自动同步（每天晚上10点）
- ✅ 移动端适配
  - 支持手机浏览，表格横向滚动

---

## 维护与更新

### 英雄资源更新

当 Dota 2 发布新英雄时，使用以下脚本更新英雄头像和映射：

```bash
cd frontend
npm run update-heroes
```

### 数据库维护

如果 Docker 部署遇到数据库问题，可以尝试重置数据卷：
```bash
docker-compose down
docker volume rm mrjz_mrjz-data
docker-compose up -d
```

### 🧹 清除数据并重新部署

如果您需要彻底清除旧数据（清空数据库）并重新部署最新代码，请在 NAS 终端或命令行中执行以下步骤：

1. **停止服务并删除容器**
   ```bash
   docker-compose down
   ```

2. **删除旧数据库卷** (这一步会永久删除所有历史数据！)
   ```bash
   docker volume rm mrjz_mrjz-data
   ```
   *注意：如果您的文件夹名不是 `mrjz`，卷名可能是 `文件夹名_mrjz-data`。可以使用 `docker volume ls` 查看准确名称。*

3. **拉取最新代码**
   ```bash
   git pull
   ```

4. **重新构建并启动**
   ```bash
   docker-compose up -d --build
   ```

---

## 📅 版本历史
### v1.1.2 (2026-03-28)
- **第三届比赛支持**:
  - 添加联赛编号 19483（第三届）到下拉选择器
  - 所有页面（比赛列表、选手统计、战队列表）默认显示第三届数据
  - **部署提示**: 如需同步第三届比赛数据，请将 `.env` 或 `docker-compose.yml` 中的 `LEAGUE_ID` 更新为 `19483`

### v1.1.1 (2026-02-02)
- **隐藏成就系统** - "乐邦詹士"（LeBron James）
  - 达成条件：这是一个秘密，只有完成的人才知道
  - 唯一性：每届联赛只有第一个完成的玩家获得
  - 特殊标识：金色王冠图标 👑
- **UI 增强**
  - **王冠图标组件** (`CrownIcon`)
    - 金色王冠图标，带呼吸式发光动画
    - 精美的 Hover 提示效果
    - 位置：玩家头像正上方居中，部分重叠
    - 尺寸：28px，醒目可见
- **成就列表优化**
  - 独立的"隐藏成就"展示区域
  - 金色主题设计（渐变背景、边框、文字）
  - 成就条件 Hover 显示（更简洁的默认视图）
  - 卡片 Hover 动画效果

### v1.1.0 (2026-02-02)
- **成就系统增强**:
  - 完成了"让让你们的呀"成就的实现（摧毁不朽之守护且获胜）
  - 事件检测：使用 `CHAT_MESSAGE_DENIED_AEGIS` 事件
  - 优化判定逻辑：支持多次打盾的情况，只要获胜队伍有任意一次摧毁不朽之守护即可达成
  - 通过实际比赛数据（8667998360）验证功能正确性
- **开源协议**:
  - 添加了 MIT License 开源协议
  - 更新 README 许可证说明，明确使用权限

### 2026-01-29 热修复补丁
- **数据映射修正**: 修正了 Snapfire (128) 和 Muerta (138) 的英雄 ID 映射错误。
- **资源加载统一**: 比赛列表页（含移动端）统一采用新的头像获取逻辑，修复百戏大王 (Ringmaster) 头像不显示的问题。
- **界面文案优化**: 修正了移动端选手列表排序下拉框默认显示为 "total_match" 变量名的问题，现已正确显示为中文 "场次"。

### v1.0.4 (2026-01-29)
- **移动端全面优化**:
  - **战队列表**: 全新卡片式设计，更紧凑的信息展示，双列队员网格。
  - **战队详情**: 优化头部布局，移除联赛筛选（默认显示全部），比赛记录转为垂直卡片流。
  - **比赛详情**: 修复表格溢出问题，移动端采用自适应布局。
  - **交互体验**: 统一了下拉框样式，优化了长文本截断和对齐。
- **Bug 修复**:
  - **新英雄支持**: 修复了凯和百戏大王头像显示错误的问题（更新本地映射）。
  - **数据展示**: 修复了移动端部分统计数据（如胜场）显示冗余的问题。

### v1.0.3 (2026-01-29)
- **新功能**:
  - **选手详情页**: 全新设计的选手个人主页 (`/players/:id`)，展示胜率、KDA、历史比赛等。
  - **数据增强**: 选手列表和比赛详情新增“建筑伤害”数据。
  - **擅长英雄**: 选手列表和个人页展示所有“绝活英雄”（按胜场数优先排序）。
  - **动态战队名**: 个人页根据最新比赛显示选手所属战队真实名称。

### v1.0.2 (2026-01-28)
- **UI 优化**: 升级了选手页面的 UI 展示（添加战队标识）

### v1.0.1 (2026-01-28)
- **新特性: 选手统计页面**:
  - 全新的选手数据列表 (`/players`)
  - 完整的数据展示：KDA、胜率、GPM/XPM、场均伤害/承伤等
- **UI 优化**:
  - 统一了所有页面的空数据状态 (`NoData` 组件)
  - 优化了深色模式下的背景显示
- **后端增强**:
  - 数据库新增 `damage_taken`（承伤）字段支持
  - 优化分页与数据聚合逻辑

### v1.0.0 (2026-01-26)
- **All-in-One 架构**: 单容器部署所有服务
- **移动端适配**: 优化手机端浏览体验
- **自动化**: 自动修复数据库兼容性问题


## 许可证

本项目采用 [MIT License](LICENSE) 开源协议。

这意味着你可以自由地：
- ✅ 使用本项目代码
- ✅ 修改和定制功能
- ✅ 用于商业或非商业目的
- ✅ 分发和再授权

唯一的要求是在使用时保留原始的版权声明和许可声明。
