...
### v1.0.0 (2026-01-26) - 初始化正式版
- **核心架构**：采用 All-in-One Docker 单容器架构，集成 Nginx、Node.js 和 MariaDB。
- **移动端适配**：全面支持手机浏览器，表格支持横向滚动，布局自适应。
- **自动化**：实现联赛数据自动同步、增量更新和定时任务。
- **功能特性**：
    - 完整的比赛详情页（KDA、出装、经济、伤害）。
    - 自动成就系统（MVP、暴走、超神等）。
    - 选手数据统计与英雄池分析。
    - 数据库自动修复与备份兼容性支持。
