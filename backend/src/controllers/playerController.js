        ...
        try {
            ...
            const sql = `
                SELECT 
                    ...
                    SUBSTRING_INDEX(GROUP_CONCAT(COALESCE(CASE WHEN mp.team = 'radiant' THEN m.radiant_team_name ELSE m.dire_team_name END, '') ORDER BY m.start_time DESC SEPARATOR '|||'), '|||', 1) as team_name
                FROM match_players mp
                JOIN matches m ON mp.match_id = m.match_id
